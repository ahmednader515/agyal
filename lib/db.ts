...
export async function createActivationCodes(
  ...
  quizIds?: string[] | null,
  grantsWhiteboard?: boolean
): Promise<{ id: string; code: string }[]> {
  const created: { id: string; code: string }[] = [];
  const seen = new Set<string>();
  const whiteboardOnly = grantsWhiteboard === true;
  const scopedLessonIds = whiteboardOnly
    ? []
    : Array.isArray(lessonIds)
      ? Array.from(new Set(lessonIds.filter((x) => typeof x === "string" && x.trim()).map((x) => x.trim())))
      : [];
  const scopedQuizIds = whiteboardOnly
    ? []
    : Array.isArray(quizIds)
  ...
  for (let i = 0; i < count; i++) {
    ...
    const id = generateId();
    await sql`
      INSERT INTO "ActivationCode" (id, course_id, code, used_at, used_by_user_id, grants_whiteboard)
      VALUES (${id}, ${courseId}, ${code}, NULL, NULL, ${whiteboardOnly})
    `;
...
export async function getActivationCodeByCode(
  code: string
): Promise<(ActivationCode & { courseId: string; lessonIds: string[]; quizIds: string[]; grantsWhiteboard: boolean }) | null> {
  ...
  const r = rows[0] as Record<string, unknown> | undefined;
  if (!r) return null;
  const base = rowToCamel(r) as ActivationCode & { courseId: string; grantsWhiteboard?: boolean };
  const grantsWhiteboard = base.grantsWhiteboard === true || base.grants_whiteboard === true;
  try {
    ...
    return { ...base, lessonIds, quizIds, grantsWhiteboard };
  } catch {
    return { ...base, lessonIds: [], quizIds: [], grantsWhiteboard };
  }
}
...
export async function useActivationCode(
  codeId: string,
  userId: string
): Promise<{ courseId: string; lessonIds: string[]; quizIds: string[]; grantsWhiteboard: boolean } | null> {
  // تحديث آمن لمنع تفعيل نفس الكود مرتين
  const updated = await sql`
    UPDATE "ActivationCode"
    SET used_at = NOW(), used_by_user_id = ${userId}
    WHERE id = ${codeId} AND used_at IS NULL
    RETURNING course_id, grants_whiteboard
  `;
  const row = updated[0] as { course_id?: string; grants_whiteboard?: boolean } | undefined;
  ...
  const grantsWhiteboard = row.grants_whiteboard === true;

  // إذا لم يتم تحديد حصص ولا اختبارات ولم يكن كود سبورة => تسجيل كامل في الدورة (السلوك القديم)
  if (lessonIds.length === 0 && quizIds.length === 0 && !grantsWhiteboard) {
    await createEnrollment(userId, row.course_id);
  }
  return { courseId: row.course_id, lessonIds, quizIds, grantsWhiteboard };
}

/** Whether a student redeemed a whiteboard-only promo code for this course. */
export async function userHasWhiteboardAccessForCourse(userId: string, courseId: string): Promise<boolean> {
  try {
    const rows = await sql`
      SELECT 1 FROM "ActivationCode"
      WHERE course_id = ${courseId}
        AND used_by_user_id = ${userId}
        AND grants_whiteboard = true
...
export async function createLiveStream(data: {
  ...
  const id = generateId();
  await sql`
    INSERT INTO "LiveStream" (id, course_id, title, title_ar, provider, meeting_url, meeting_id, meeting_password, scheduled_at, description, "order", whiteboard_enabled)
    VALUES (${id}, ${data.course_id}, ${data.title}, ${data.title_ar ?? null}, ${data.provider}, ${data.meeting_url}, ${data.meeting_id ?? null}, ${data.meeting_password ?? null}, ${data.scheduled_at}, ${data.description ?? null}, ${data.order ?? 0}, ${data.whiteboard_enabled ?? true})
  `;
...
export async function updateLiveStream(
  ...
  if (data.whiteboard_enabled !== undefined) await sql`UPDATE "LiveStream" SET whiteboard_enabled = ${data.whiteboard_enabled}, updated_at = NOW() WHERE id = ${id}`;
  if (data.whiteboard_snapshot_url !== undefined) await sql`UPDATE "LiveStream" SET whiteboard_snapshot_url = ${data.whiteboard_snapshot_url}, updated_at = NOW() WHERE id = ${id}`;
}

export async function updateLiveStreamWhiteboardSnapshot(id: string, snapshotUrl: string): Promise<void> {
  await sql`UPDATE "LiveStream" SET whiteboard_snapshot_url = ${snapshotUrl}, updated_at = NOW() WHERE id = ${id}`;
}
