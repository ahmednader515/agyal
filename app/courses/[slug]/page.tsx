              ...
              <div className="mt-8">
                ...
                <ul className="mt-4 space-y-3">
                  {(liveStreams as unknown as Record<string, unknown>[]).map((ls) => {
                    const streamId = String(ls.id);
                    const whiteboardEnabled = ls.whiteboard_enabled !== false && ls.whiteboardEnabled !== false;
                    const courseSegment =
                      (course.slug && String(course.slug).trim())
                        ? encodeURIComponent(String(course.slug).trim())
                        : course.id;
                    return (
                    <li
                      key={streamId}
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {whiteboardEnabled && canAccessContent && (
                          ...
                          >
                            {t("courses.enterLiveRoom", "Enter live room")}
                          </Link>
                        )}
                        <a
                          ...
                          className="rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-2 text-sm font-medium text-[var(--color-foreground)] hover:bg-[var(--color-border)]/50"
                        >
