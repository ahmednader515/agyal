...
export function LiveStreamForm({ courseOptions, initialData }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    whiteboardEnabled: initialData?.whiteboardEnabled ?? true,
  });
    ...
    <form onSubmit={handleSubmit} className="mt-6 max-w-xl space-y-4">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          type="checkbox"
          checked={form.whiteboardEnabled}
          onChange={(e) => setForm((f) => ({ ...f, whiteboardEnabled: e.target.checked }))}
          className="h-4 w-4 rounded border-[var(--color-border)]"
        />
        <label htmlFor="whiteboardEnabled" className="text-sm font-medium text-[var(--color-foreground)]">
          {t(`${F}.whiteboardEnabled`)}
        </label>
      </div>
      <button
        type="submit"
